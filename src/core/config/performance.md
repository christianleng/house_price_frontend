...
### Case Study: Optimisation useIsMobile (206ms → 135ms)

#### Le Problème

Lors du profilage avec Chrome DevTools, j'ai découvert une long task de 206ms au chargement de la page. En analysant le flame graph du React Profiler, j'ai remarqué que `RootLayout` montait **deux fois** dans le même cycle de rendu - un signe clair d'une cascade de re-renders.

![Long task before optimization](../../../public/docs/long-task-before.png)

#### Le Diagnostic (DevTools Long task ↑ 206 ms)
...
Total: ~50ms pour le mount initial
       + cascades minimisées
       = 135ms long task (vs 206ms avant)
       = 34,5% d'amélioration!
```
...
| --------------------- | ----- | ----- | ------------ |
| Long Task             | 206ms | 135ms | -34,5%       |
| RootLayout mounts     | 2     | 1     | -50%         |
| MainNavigation render | 55ms  | ~25ms | -55%         |
| Total navigation time | 206ms | 135ms | -34,5%       |
...
- Vérification: Lighthouse → 135ms (ou 50ms avec memoization)
